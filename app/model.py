...
def _get_user_by_id(conn, id: int) -> Optional[SafeUser]:
    result = conn.execute(
        text("SELECT id, name, leader_card_id FROM user WHERE id = :id"), {"id": id}
    )
    try:
        user = result.one()
    except NoResultFound:
...
def _create_member(
    conn, room_id: int, member_id: int, select_difficulty: LiveDifficulty
):
    conn.execute(
        text(
...
def create_room(owner_id: int, live_id: int, select_difficulty: LiveDifficulty) -> int:
    with engine.begin() as conn:
...
def _get_rooms_by_live_id(conn, live_id: int) -> list[Room]:
    if live_id == 0:
        result = conn.execute(
            text("SELECT * FROM room WHERE wait_room_status = :waiting"),
...
def _get_room_by_id(conn, room_id: int) -> Optional[Room]:
    result = conn.execute(text("SELECT * FROM room WHERE id = :id"), {"id": room_id})
    try:
        room = result.one()
    except NoResultFound:
        return None
...
def _get_room_members_count_by_room_id_with_lock(conn, room_id: int) -> int:
    result = conn.execute(
        text("SELECT COUNT(*) FROM room_member WHERE room_id = :room_id FOR UPDATE"),
        {"room_id": room_id},
    )
    return result.scalar()
...
def add_member(
    room_id: int, member_id: int, select_difficulty: LiveDifficulty
) -> JoinRoomResult:
    with engine.begin() as conn:
        ...
        membersCount = _get_room_members_count_by_room_id_with_lock(conn, room_id)
...
def _update_room_status(conn, room_id: int, status: WaitRoomStatus):
    conn.execute(
        text("UPDATE room SET wait_room_status = :status WHERE id = :id"),
...
def _get_room_by_id_with_lock(conn, room_id: int) -> Optional[Room]:
    result = conn.execute(
        text("SELECT * FROM room WHERE id = :id FOR UPDATE"), {"id": room_id}
    )
    try:
        room = result.one()
    except NoResultFound:
...
def _update_result(
    conn, user_id: int, room_id: int, judge_count_list: list[int], score: int
):
...
def finish_game(user_id: int, room_id: int, judge_count_list: list[int], score: int):
    with engine.begin() as conn:
...
def _update_room_owner(conn, id: int, owner_id: int):
    conn.execute(
        text("UPDATE room SET owner_id = :owner_id WHERE id = :id"),
...
def delete_member(room_id: int, member_id: int):
    with engine.begin() as conn:
        ...
        if all(member.member_id != member_id for member in members):
            raise HTTPException(
                status_code=404, detail="member is not found in the room"
            )
